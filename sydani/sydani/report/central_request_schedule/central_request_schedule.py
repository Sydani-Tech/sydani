...
def execute(filters=None):
    columns = [
        ...
        {"label": "Total Paid", "fieldname": "total_paid", "fieldtype": "Float", "width": 150},
        {"label": "Percentage Paid", "fieldname": "percentage_paid", "fieldtype": "Percent", "width": 130},
        {"label": "Total Raised", "fieldname": "total_raised", "field_type": "Float", "width": 150},
        {"label": "Percentage Raised", "fieldname": "percentage_raised", "fieldtype": "Percent", "width": 150},
    ...
    max_percentage_paid = filters.get("max_percentage_paid") if filters else 94
    ...
    if request_type == "All" or request_type == "Expense Claim":
        expense_claims = frappe.get_all(
            "Expense Claim",
            filters={
                "workflow_state": ["in", ["Approved", "Retirement Approved", "Authorized By Managing Partner", "Payment Recorded", "Advance Request Approved", "Advance Retirement Approved", "Advance Retired By Employee", "Advance Retirement Validated By Finance", "Advance Retirement Verified By Audit", "Advance Retirement Authorized By Manager", "Retirement Returned To Employee"]],
                # "date_created": ["between", date_filter],
                "date_created": date_filter,
                "suspend_payment": 0,
                **({"project": project} if project else {}),
                **({"company": company} if company else {}),
            },
            fields=["name", "creation", "purpose", "record_status", "manager_name", "project", "payment_total", "payment_total_advance", "currency", "company"],
    ...
    if request_type == "All" or request_type == "Employee Travel Request":
        travel_requests = frappe.get_all(
            ...
            filters={
                "workflow_state": ["in", ["Approved By Manager", "Change Request Approved", "Travel Request Finalized"]],
                # "trip_start_date": ["between", date_filter],
                "trip_start_date": date_filter,
                **({"project_name": project} if project else {}),
                **({"company": company} if company else {}),
            },
            fields=["name", "creation", "record_status", "purpose_of_the_trip", "project_name", "total", "currency", "company"]
        )
    ...
    if request_type == "All" or request_type == "Procurement Request":
        procurement_requests = frappe.get_all(
            "Procurement Request",
            filters={
                "workflow_state": ["in", ["Approved By Manager", "Change Request Approved"]],
                # "request_date": ["between", date_filter],
                "request_date": date_filter,
                **({"project": project} if project else {}),
                **({"company": company} if company else {}),
            },
            fields=["name", "purpose", "project", "total", "currency", "company"]
        )
    ...
    if request_type == "All" or request_type == "Procurement Evaluation Sheet":
        procurement_purchase_orders = frappe.get_all(
            ...
            filters={
                "workflow_state": ["in", ["Approved", "Payment Processed", "Draft"]],
                "creation": date_filter,
                **({"project": project} if project else {}),
                **({"company": company} if company else {}),
            },
            fields=["name", "creation", "record_status",  "purpose", "project", "grand_total", "currency", "company"]
        )
    ...
    if request_type == "All" or request_type == "Capacity Building Fund Request":
        capacity_building_requests = frappe.get_all(
            ...
            filters={
                "workflow_state": ["in", ["Approved", "Completed", "Draft"]],
                # "creation": ["between", date_filter],
                "creation": date_filter,
                **({"project": project} if project else {}),
                **({"company": company} if company else {}),
            },
            fields=["name", "creation", "status", "employee", "coursetraining_title", "institution","cost", "currency", "company"]
        )
